...
def handle_dialog(res, req):
    ...
    if sessionStorage[user_id]['first_name'] is None:
        ...
        if sessionStorage[user_id]['action'] is None:
            ...
            if yes_or_no(req):
                res['response']['text'] = 'Прекрсно! Выбери Данетку: ' + str([e for e in Danetki.keys()])[2:-2]
...
def select(req, res):
    if req['request']['original_uttarance'] in Danetki:
        return req['request']['original_uttarance']
    else:
